...
def analyze_usb_parquet(parquet_file, show_payload_samples=False):
    ...
    print(f"Payload length range: {df['data_length'].min()} - {df['data_length'].max()} bytes")
    print(f"Average payload length: {df['data_length'].mean():.2f} bytes\n")
    
    # Direction analysis
    print("=== Direction Analysis ===")
    direction_counts = df.group_by('direction').agg(pl.len().alias('count')).sort('count', descending=True)
    print(direction_counts)
    print()
    
    # Endpoint analysis
    print("=== Endpoint Analysis ===")
    endpoint_counts = df.group_by('endpoint_address').agg([
        pl.len().alias('count'),
    ...
    length_counts = df.group_by('data_length').agg(pl.len().alias('count')).sort('data_length')
    ...
    df_with_second = df.with_columns([
        pl.col('timestamp').floor().alias('second')
    ])
    
    traffic_per_second = df_with_second.group_by('second').agg([
        pl.len().alias('packets_per_second'),
        pl.col('data_length').sum().alias('bytes_per_second')
    ]).sort('second')
    
    print(f"Peak traffic: {traffic_per_second['packets_per_second'].max()} packets/second")
    print(f"Average traffic: {traffic_per_second['packets_per_second'].mean():.2f} packets/second")
    print(f"Peak bandwidth: {traffic_per_second['bytes_per_second'].max()} bytes/second")
    print(f"Average bandwidth: {traffic_per_second['bytes_per_second'].mean():.2f} bytes/second\n")
    ...
    payload_stats = df.group_by(['direction', 'data_length']).agg([
        pl.len().alias('count'),
    ...
    if show_payload_samples:
        ...
        for length in sorted(df['data_length'].unique().to_list())[:10]:  # First 10 lengths
            ...
            for row in samples.iter_rows(named=True):
                ...
                try:
                    ...
                    ascii_repr = ''.join(chr(b) if 32 <= b <= 126 else '.' for b in payload_bytes)
...
def find_patterns(df):
    ...
    common_payloads = df.group_by('payload_hex').agg([
        pl.len().alias('count'),
...
def export_payload_data(df, output_file):
    ...
    export_df = df.with_columns([
        ...
        pl.col('payload_hex').map_elements(
        ...
        pl.col('payload_hex').map_elements(
            lambda hex_str: ''.join(chr(b) if 32 <= b <= 126 else '.' for b in bytes.fromhex(hex_str)) if hex_str else '',
...
def main():
    parser = argparse.ArgumentParser(description='Analyze USB packet data from Rust-generated Parquet files')
    parser.add_argument('parquet_file', help='Input Parquet file from Rust converter')
    parser.add_argument('--show-payload-samples', action='store_true', 
                       help='Show sample payload data')
    parser.add_argument('--export', type=str, 
                       help='Export processed data to file (specify base name)')
    ...
    try:
        ...
        print("The Rust converter successfully extracted raw USB payload data that pyshark couldn't access.")
