...
- PD events: `PdEvent.data` is a one-key variant dict (`Connect`, `Disconnect`, `PdMessage`). Use `iter_pd_messages`, `pd_event_kind`, `pd_message_sop` and `pd_message_wire` from `km003c_analysis.helpers`; never read `event.data` or a `wire_data` attribute directly. Decode a sequence of wire messages with one `usbpdpy.PdDecoder` per connection, fed in capture order and `reset()` on Connect/Disconnect: it resolves Requests against the last Source_Capabilities and reassembles chunked EPR_Source_Capabilities (`decode()` returns `None` for an unfinished chunk). `parse_pd_message` is only for a single, self-contained message.
