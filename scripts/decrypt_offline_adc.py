...
def main():
    ...
    for row in bulk_in.iter_rows(named=True):
        ...
        if payload_hex:
            ...
            if pkt_type == 0x44 and len(payload) == 20:  # Unknown68 response
                ...
                print(
                    f"  Frame {row['frame_number']}: type=0x{pkt_type:02X}, bit16={bit16}"
                )
                ...
                if len(payload) >= 20:
                    ...
                    print(
                        f"    Parsed: addr=0x{addr:08X}, size={size}, marker=0x{marker:08X}, crc=0x{crc:08X}"
                    )
    ...
    for row in bulk_in.iter_rows(named=True):
        ...
        if payload_hex and len(payload_hex) > 1000:  # > 500 bytes
            ...
            print(
                f"  Frame {row['frame_number']}: type=0x{pkt_type:02X}, size={len(payload)}"
            )
    ...
    for frame, payload in large_chunks:
        ...
        print(
            f"  Frame {frame}: type=0x{pkt_type:02X}, tid={tid}, flags=0x{flags:04X}, bit16={bit16}"
        )
    ...
    for field_idx in range(4):
        ...
        print(
            f"Field {field_idx}: min={min_v:>12}, max={max_v:>12}, avg={avg_v:>12.0f}"
        )
        if 1_000_000 < avg_v < 30_000_000:
            print(
                f"         -> Likely VOLTAGE: {avg_v / 1e6:.3f} V (range {min_v / 1e6:.3f} - {max_v / 1e6:.3f} V)"
            )
        elif abs(avg_v) < 10_000_000 and avg_v != 0:
            print(
                f"         -> Likely CURRENT: {avg_v / 1e6:.6f} A ({avg_v / 1e3:.3f} mA)"
            )

    # Show first 10 and last 10 samples
    print("\n--- First 10 Samples ---")
    print(
        f"{'#':>4} {'Voltage(V)':>12} {'Current(A)':>12} {'Field2':>12} {'Field3':>12}"
    )
    ...
    for field_idx in range(4):
        ...
        if all(d >= 0 for d in diffs) and max(diffs) > 0:
            ...
            print(
                f"  Min delta: {min(diffs)}, Max delta: {max(diffs)}, Avg delta: {sum(diffs) / len(diffs):.0f}"
            )
