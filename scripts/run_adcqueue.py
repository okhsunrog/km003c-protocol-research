...
class KM003C:
    ...
    def send_cmd(self, cmd_type, data_word):
        ...
        packet = bytes(
            [cmd_type & 0x7F, self.tid, data_word & 0xFF, (data_word >> 8) & 0xFF]
        )

        return self.send_packet(packet)

    def send_packet(self, packet):
        """Send an encoded packet and return its response."""
    ...
    def start_graph(self, rate_index):
        ...
        self.tid = (self.tid + 1) & 0xFF
        packet = create_packet(CMD_START_GRAPH, self.tid, rate_index)
        response = self.send_packet(packet)

        if response:
            resp_type = response[0] & 0x7F
            print(
                f"  Device response: type=0x{resp_type:02X}, {len(response)} bytes: {response.hex()}"
            )
...
def main():
    ...
    try:
        ...
        for i in range(10):
            ...
            if response:
                print(
                    f"Request {i + 1:2d}: Got {len(response)} bytes: {response[:16].hex() if len(response) >= 16 else response.hex()}"
                )
            else:
            ...
            if len(response) >= 16:
                ...
                if attr == 2:  # AdcQueue
                    ...
                    print(
                        f"Request {i + 1:2d}: ✓ {size:4d} bytes = {num_samples:2d} samples"
                    )
                elif attr == 1:  # ADC
                    print(
                        f"Request {i + 1:2d}: → ADC data ({size} bytes, not AdcQueue yet)"
                    )
                else:
