...
def main():
    ...
    for address, size, description in mcu_registers:
        ...
        if responses:
            for i, resp_bytes in enumerate(responses):
                ...
                if resp_type == 0x06:
                    print("  -> REJECTED (address may not be readable)")
                    ...
                    print(
                        f"  -> Response type 0x{resp_type:02X}: {resp_bytes[:32].hex()}"
                    )
        else:
            print("  -> TIMEOUT")
    ...
    if 0xE000ED00 in results and len(results[0xE000ED00]) >= 4:
        ...
        print(
            f"  Implementer: 0x{implementer:02X} ({'ARM' if implementer == 0x41 else 'Unknown'})"
        )
        print(f"  Variant: {variant}")
        print(
            f"  Architecture: 0x{architecture:X} ({'ARMv7-M' if architecture == 0xF else 'ARMv6-M' if architecture == 0xC else 'Unknown'})"
        )
