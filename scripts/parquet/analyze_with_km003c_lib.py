...
def analyze_with_rust_lib():
    ...
    print(
        f"Successfully parsed: {stats['parse_success']:,} ({stats['parse_success'] / stats['total_packets'] * 100:.1f}%)"
    )
    ...
    for pkt_type, count in sorted(
        stats["packet_types"].items(), key=lambda x: x[1], reverse=True
    ):
    ...
    for mask, count in sorted(
        stats["attributes_in_requests"].items(), key=lambda x: x[1], reverse=True
    ):
        # Decode mask
        mask_val = int(mask, 16)
        bits_set = []
        if mask_val & 0x0001:
            bits_set.append("ADC(1)")
        if mask_val & 0x0002:
            bits_set.append("AdcQueue(2)")
        if mask_val & 0x0008:
            bits_set.append("Settings(8)")
        if mask_val & 0x0010:
    ...
    for attrs, count in sorted(
        stats["attributes_in_responses"].items(), key=lambda x: x[1], reverse=True
    ):
    ...
    if stats["adc_data_samples"]:
        ...
        print(
            f"    Example: VBUS={first_adc['vbus_v']:.3f}V, IBUS={first_adc['ibus_a']:.3f}A, Power={first_adc['power_w']:.3f}W"
        )
    ...
    if stats["pd_status_samples"]:
        ...
        print(
            f"    Example: timestamp={first_pd['timestamp']}, VBUS={first_pd['vbus_v']:.3f}V"
        )
