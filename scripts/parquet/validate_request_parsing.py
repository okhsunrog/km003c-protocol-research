...
def test_request_parsing_with_state():
    ...
    for i, pdo in enumerate(source_msg.data_objects):
        print(
            f"  PDO{i + 1}: {pdo.pdo_type} {pdo.voltage_v}V @ {pdo.max_current_a}A = {pdo.max_power_w}W"
        )
    print()
    ...
    try:
        ...
        if basic_request.request_objects:
            ...
            print(
                f"  RDO without state: position={rdo.object_position}, type={rdo.rdo_type}"
            )
    ...
    try:
        enhanced_request = usbpdpy.parse_pd_message_with_state(
            request_bytes, source_msg.data_objects
        )
        ...
        if enhanced_request.request_objects:
            ...
            if 1 <= rdo.object_position <= len(source_msg.data_objects):
                ...
                print(
                    f"    └─ {requested_pdo.pdo_type}: {requested_pdo.voltage_v}V @ {requested_pdo.max_current_a}A = {requested_pdo.max_power_w}W"
                )
...
def test_negotiation_sequence():
    ...
    request_msg = usbpdpy.parse_pd_message_with_state(
        bytes.fromhex(messages[1]), source_msg.data_objects
    )
    print(
        f"2. {request_msg.header.message_type}: Requesting PDO #{request_msg.request_objects[0].object_position}"
    )
