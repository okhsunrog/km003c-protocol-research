...
def analyze_km003c_protocol() -> None:
    ...
    print("Using usbpdpy v0.2.0")
    print(
        f"KM003C Rust library: {'✅ Available' if KM003C_LIB_AVAILABLE else '❌ Not available'}"
    )
    print()
    ...
    if pd_candidates:
        ...
        source_caps = [
            c
            for c in pd_candidates
            if c["pd_result"].message_type == "Source_Capabilities"
        ]
        if source_caps:
            print("\n=== SOURCE CAPABILITIES ANALYSIS ===")
            ...
            if first_caps.pdos:
                ...
                for i, pdo in enumerate(first_caps.pdos):
                    ...
                    print(
                        f"  PDO{i + 1}: {pdo['pdo_type']} - {pdo['voltage_v']}V @ {pdo['max_current_a']}A = {pdo['max_power_w']}W{extra_info}"
                    )

        # Request Analysis
        requests = [
            c for c in pd_candidates if c["pd_result"].message_type == "Request"
        ]
        if requests:
            ...
            for req in requests:
                ...
                if req_result.rdos:
                    ...
                    if rdo.get("operating_current_a"):
                        print(
                            f"    └─ Operating current: {rdo['operating_current_a']}A"
                        )
                    if rdo.get("capability_mismatch"):
                        print("    └─ ⚠️  Capability mismatch")

        # Protocol Flow Analysis
        control_msgs = [
            c
            for c in pd_candidates
            if c["pd_result"].message_type in ["GoodCRC", "Accept", "PS_RDY"]
        ]
        if control_msgs:
            print("\n=== PROTOCOL FLOW ANALYSIS ===")
    ...
    if current_source_capabilities:
        print(
            f"Source Capabilities PDOs for Request parsing: {len(current_source_capabilities)}"
        )

    print("\n✅ Analysis complete!")
    print(
        "The KM003C protocol analysis now includes complete PD message parsing with usbpdpy v0.2.0"
    )
