...
def parse_logical_packets(hex_data: str) -> List[LogicalPacket]:
    ...
    try:
        ...
        for lp in raw["Data"].get("logical_packets", []) or []:
            ...
            payload_hex = (
                payload.hex() if isinstance(payload, (bytes, bytearray)) else ""
            )
        ...
        if len(request_packets) > 0 and len(response_packets) > 0:
            ...
            if req_header and resp_header:
                ...
                if req_header.transaction_id == resp_header.transaction_id:
                    ...
                    # Calculate latency
                    latency_us = (
                        resp_row["timestamp"] - req_row["timestamp"]
                    ) * 1_000_000
...
def analyze_attribute_mapping(pairs: List[TransactionPair]) -> Dict[str, Any]:
    ...
    result = {"summary": {}, "detailed_mapping": {}, "bit_analysis": {}}
...
def analyze_per_source_file() -> None:
    ...
    print("=" * 80)
    print(
        f"KM003C Rust library: {'✅ Available' if KM003C_LIB_AVAILABLE else '❌ Not available (using manual parsing)'}"
    )
    print()
    ...
    for source_file in source_files:
        ...
        for mask_hex, summary in mapping_analysis["summary"].items():
            print(
                f"      {mask_hex}: {summary['total_occurrences']} occurrences → {summary['most_common_response']}"
            )
    ...
    if len(all_pairs) > 0:
        ...
        for mask_hex, summary in sorted(global_mapping["summary"].items()):
            ...
            if summary["unique_response_patterns"] > 1:
                print("    Alternate patterns:")
                for pattern, count in global_mapping["detailed_mapping"][
                    mask_hex
                ].items():
            ...
            pairs_data.append(
                {
                    ...
                    "response_attributes": str(
                        [lp.attribute for lp in pair.logical_packets]
                    ),
