...
class TestPacketParsing:
    ...
    def test_parse_generic_data_command(self):
        ...
        extended_command_hex = "4402010133f8860c0054288cdc7e52729826872dd18b539a39c407d5c063d91102e36a9e"
    ...
    def test_adc_response_transaction_ids(self):
        ...
        test_cases = [
            ("410a82020100000b5c0f0000faffffffa00f0000f2ffffff04100000500000007e0d7b7ed40471014201837e0080780025002100", 0x0A),
            ("410b82020100000ba70f0000f6ffffff810f0000f8ffffffe50f0000560000007f0d7b7ed40471014201837e0080780024002000", 0x0B),
            ("410c82020100000ba70f0000f6ffffff810f0000f8ffffffe50f0000560000007f0d7b7ed30475014701837e0080780024001f00", 0x0C),
        ]
...
class TestPacketClassification:
    ...
    def test_all_packet_types_covered(self):
        ...
        test_cases = [
            ...
            ("410a82020100000b5c0f0000faffffffa00f0000f2ffffff04100000500000007e0d7b7ed40471014201837e0080780025002100", "SimpleAdcData", "ADC response data"),
            ("02010000", "Generic", "Control command"),
            ("0c071000", "Generic", "Non-ADC GetData command"),
            ("4402010133f8860c0054288cdc7e52729826872dd18b539a39c407d5c063d91102e36a9e", "Generic", "Extended data command"),
        ]
        
        for hex_data, expected_type, description in test_cases:
            packet_bytes = bytes.fromhex(hex_data)
            result = parse_packet(packet_bytes)
            assert result.packet_type == expected_type, f"Failed for {description}: {hex_data}"
