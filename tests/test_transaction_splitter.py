...
def test_splitter_preserves_data_per_source(source_dataframes):
    ...
    for source_file, source_df in source_dataframes:
        ...
        # 2. Verify row count is unchanged
        assert source_df.height == result_df.height, f"Row count changed for {source_file}"

        # 3. Verify original columns are preserved
        original_cols = source_df.columns
        result_cols_without_tid = [col for col in result_df.columns if col != "transaction_id"]
        assert original_cols == result_cols_without_tid, f"Columns changed for {source_file}"
        ...
        assert sorted_source_df.equals(result_df.select(original_cols)), f"Data modified for {source_file}"
...
def test_splitter_creates_valid_transactions_per_source(source_dataframes):
    ...
    for source_file, source_df in source_dataframes:
        ...
        # Check that the transaction_id column exists
        assert "transaction_id" in result_df.columns, f"No transaction_id for {source_file}"
        ...
        assert result_df["transaction_id"].dtype == pl.Int64, f"Wrong dtype for {source_file}"
