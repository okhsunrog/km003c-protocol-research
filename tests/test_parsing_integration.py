...
class TestPacketAnalysisHelpers:
    ...
    def test_extract_transaction_id(self):
        ...
        test_cases = [
            ...
            ('410a82020100000b5c0f0000faffffffa00f0000f2ffffff04100000500000007e0d7b7ed40471014201837e0080780025002100', 0x0A),  # ADC response
    ...
    def test_packet_size_analysis(self):
        ...
        test_cases = [
            ...
            ('410a82020100000b5c0f0000faffffffa00f0000f2ffffff04100000500000007e0d7b7ed40471014201837e0080780025002100', 52, 'ADC response'),
            ('02010000', 4, 'Control command'),
            ('4402010133f8860c0054288cdc7e52729826872dd18b539a39c407d5c063d91102e36a9e', 36, 'Extended command'),
        ]
        
        for hex_data, expected_size, description in test_cases:
            packet_bytes = bytes.fromhex(hex_data)
            assert len(packet_bytes) == expected_size, f"Size mismatch for {description}"
            
            # Parse and verify packet type makes sense for size
            result = parse_packet(packet_bytes)
            if expected_size == 52:
                assert result.packet_type == 'SimpleAdcData'
            elif expected_size == 4 and hex_data.startswith('0c') and hex_data.endswith('0200'):
