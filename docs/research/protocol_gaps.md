...
| Attribute `0x0020` | V1.9.9 firmware plus framed empty, active, connect/disconnect, and chained hardware responses | Two length-prefixed queues; Type-C states `0x00..0x24` are named, while `0x25` and the protocol-event enumeration remain unknown | Map protocol-event codes and compare other firmware versions |
