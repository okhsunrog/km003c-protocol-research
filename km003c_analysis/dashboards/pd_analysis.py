...
def main():
    ...
    selected_messages = st.sidebar.multiselect(
        "Message Types", message_types, default=message_types
    )

    # Negotiation filter
    negotiations = sorted(df["negotiation_id"].dropna().unique())
    selected_negotiations = st.sidebar.multiselect(
        "Negotiations", negotiations, default=negotiations
    )
    ...
    with col1:
        ...
        # Add voltage trace
        voltage_data = filtered_df.dropna(subset=["vbus_v"])
        ...
        for msg_type in ["Source_Capabilities", "Request", "Accept", "PS_RDY"]:
            msg_data = filtered_df[filtered_df["pd_message_type"] == msg_type]
    ...
    with col2:
        ...
        msg_counts = filtered_df["pd_message_type"].value_counts()
        ...
        st.metric("Negotiations", int(filtered_df["negotiation_id"].nunique() or 0))

        voltage_change = filtered_df["vbus_v"].max() - filtered_df["vbus_v"].min()
        st.metric("Voltage Range", f"{voltage_change:.2f}V")

    # PDO Analysis Section
    st.header("🔋 Power Data Object (PDO) Analysis")

    pdo_data = filtered_df[filtered_df["pdo_type"].notna()].copy()
    ...
    if not pdo_data.empty:
        ...
        with col2:
            ...
            pdo_summary = (
                pdo_data.groupby(["pdo_position", "pdo_type"])
                .agg(
                    {
                        "voltage_v": "first",
                        "max_current_a": "first",
    ...
    rdo_data = filtered_df[filtered_df["rdo_type"].notna()].copy()
    ...
    if not rdo_data.empty:
            ...
            for _, rdo in rdo_data.iterrows():
                with st.expander(f"Request at {rdo['time_s']:.3f}s"):
                    ...
                    if pd.notna(rdo["operating_current_a"]):
                        st.write(
                            f"**Operating Current:** {rdo['operating_current_a']:.2f}A"
                        )

                    st.write(f"**Capability Mismatch:** {rdo['capability_mismatch']}")
                    st.write(
                        f"**USB Communications:** {rdo['usb_communications_capable']}"
                    )
        ...
        with col2:
            ...
            if pd.notna(rdo_data["requested_voltage_v"].iloc[0]):
                ...
                # Voltage transition
                voltage_before = filtered_df["vbus_v"].iloc[0]
                voltage_after = filtered_df["vbus_v"].iloc[-1]
    ...
    flow_data = filtered_df[filtered_df["pd_message_type"].notna()].copy()
    ...
    if not flow_data.empty:
        ...
        flow_summary = flow_data[
            ["time_s", "pd_message_type", "vbus_v", "negotiation_id"]
        ].copy()
        flow_summary["time_s"] = flow_summary["time_s"].round(3)

        st.dataframe(
            flow_summary.style.format({"time_s": "{:.3f}s", "vbus_v": "{:.3f}V"}),
            use_container_width=True,
        )
    ...
    with st.expander("🔍 Raw Data Explorer"):
        ...
        with col1:
            ...
            st.download_button("Download CSV", csv_data, "pd_analysis.csv", "text/csv")

        with col2:
            # JSON export for API use
            json_data = filtered_df.to_json(orient="records", indent=2)
            st.download_button(
                "Download JSON", json_data, "pd_analysis.json", "application/json"
            )
