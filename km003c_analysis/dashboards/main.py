    ...
    try:
        ...
        if pkt_type == "GetData":
            ...
            return {
                ...
                "preview": f"GetData mask=0x{mask:04X} ({len(packet_bytes)} bytes)"
                if mask is not None
                else f"GetData ({len(packet_bytes)} bytes)",
            ...
            try:
                ...
                variant = (
                    next(iter(raw_packet.keys()))
                    if isinstance(raw_packet, dict) and raw_packet
                    else "UNKNOWN"
                )
                header = (
                    raw_packet[variant]["header"]
                    if isinstance(raw_packet, dict) and variant in raw_packet
                    else {}
                )
                return {
                    "type": "GENERIC",
        ...
        return {
            ...
            "details": {
                "error": str(e),
                "hex": hex_data[:32] + "..." if len(hex_data) > 32 else hex_data,
            },
        }
...
def load_and_process_data(
    source_file: str, hide_enumeration: bool
...
def main() -> None:
    ...
    total_pages = (
        total_transactions + rows_per_page - 1
    ) // rows_per_page  # Ceiling division
    ...
    with col1:
        ...
        st.caption(
            f"Showing transactions {start_idx + 1}-{end_idx} of {total_transactions} total"
        )
        ...
        for row in current_page_data.iter_rows(named=True):
            ...
            request_info = (
                parse_packet_preview(row["request_hex"])
                if row["request_hex"]
                else {"preview": "No request"}
            )
            response_info = (
                parse_packet_preview(row["response_hex"])
                if row["response_hex"]
                else {"preview": "No response"}
            )
            ...
            display_rows.append(
                {
                    ...
                    "request_hex": row["request_hex"][:16] + "..."
                    if row["request_hex"] and len(row["request_hex"]) > 16
                    else (row["request_hex"] or ""),
                    "response_hex": row["response_hex"][:16] + "..."
                    if row["response_hex"] and len(row["response_hex"]) > 16
                    else (row["response_hex"] or ""),
                }
            )
        ...
        selection = st.dataframe(
            ...
            column_config={
                "transaction_id": st.column_config.NumberColumn(
                    "ID", help="Transaction ID", width="small"
                ),
                ...
                "frame_count": st.column_config.NumberColumn(
                    "Frames", help="Number of frames in transaction", width="small"
                ),
                "tags": st.column_config.ListColumn(
                    "Tags", help="Transaction tags", width="large"
                ),
                "request_type": st.column_config.TextColumn(
                    "Request", help="Request packet type and preview", width="medium"
                ),
                "response_type": st.column_config.TextColumn(
                    "Response", help="Response packet type and preview", width="medium"
                ),
                "request_hex": st.column_config.TextColumn(
                    "Req Hex", help="Request hex data (truncated)", width="small"
                ),
                "response_hex": st.column_config.TextColumn(
                    "Resp Hex", help="Response hex data (truncated)", width="small"
                ),
            },
        )
    ...
    with col2:
        ...
        if not selection["selection"]["rows"]:
            ...
            selected_transaction = transactions_summary.row(
                selected_global_idx, named=True
            )
            ...
            with st.expander("📤 Request Details", expanded=True):
                if payload_data["request_data"]:
                    ...
                    try:
                        ...
                        # Show key packet details (raw_packet is a dict)
                        variant = (
                            next(iter(raw_packet.keys()))
                            if isinstance(raw_packet, dict) and raw_packet
                            else "UNKNOWN"
                        ...
                        header = (
                            raw_packet.get(variant, {}).get("header", {})
                        ...
                        with col_r2:
                            st.text(
                                f"Reserved Flag: {header.get('reserved_flag', '?')}"
                            )
                        with col_r3:
                            if variant == "Data":
                    ...
                    try:
                        ...
                        with st.expander("⚙️ Raw Packet Details", expanded=False):
                            ...
                            if isinstance(raw_packet, dict):
                                ...
                                header = (
                                    raw_packet[variant].get("header", {})
                                    if variant in raw_packet
                                    else {}
                                )
                                ...
                                if variant == "Data":
                                    ...
                                    if lps:
                                        lp0 = lps[0]
                                        st.text(
                                            "Extended Header Fields (first logical packet):"
                                        )
                    ...
                    if len(payload_data["request_frames"]) > 1:
                        st.info(
                            f"Note: {len(payload_data['request_frames'])} request frames found"
                        )
                else:
            ...
            with st.expander("📥 Response Details", expanded=True):
                if payload_data["response_data"]:
                    response_parsed = parse_packet_preview(
                    ...
                    if "parsed_packet" in response_parsed:
                        ...
                        adc = get_adc_data(packet) if isinstance(packet, dict) else None
                        if adc is not None:
                    ...
                    if len(payload_data["response_frames"]) > 1:
                        st.info(
                            f"Note: {len(payload_data['response_frames'])} response frames found"
                        )
                else:
