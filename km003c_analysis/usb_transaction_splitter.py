...
class USBTransactionSplitter:
    ...
    def is_bulk_setup(self, frame: Dict[str, Any]) -> bool:
        ...
        """
        return (frame.get(self.config.transfer_type_col) == self.config.bulk_transfer_type and 
                frame.get(self.config.endpoint_address_col) == self.config.in_endpoint and
                frame.get(self.config.urb_type_col) == self.config.submit_urb and
                frame.get(self.config.data_length_col, 0) == 0)
    ...
    def is_bulk_command_start(self, frame: Dict[str, Any]) -> bool:
        ...
        """
        return (frame.get(self.config.transfer_type_col) == self.config.bulk_transfer_type and 
                frame.get(self.config.endpoint_address_col) == self.config.out_endpoint and
                frame.get(self.config.urb_type_col) == self.config.submit_urb and
                frame.get(self.config.data_length_col, 0) > 0)
    ...
    def validate_output(self, df: pl.DataFrame) -> Dict[str, bool]:
        ...
        """
        if df.height == 0:
            return {"valid": True, "frame_order": True, "timestamp_order": True, "transaction_order": True}
        ...
        frame_order_valid = all(frame_nums[i] <= frame_nums[i+1] for i in range(len(frame_nums)-1))
        ...
        if self.config.timestamp_col in df.columns:
            ...
            timestamp_order_valid = all(timestamps[i] <= timestamps[i+1] for i in range(len(timestamps)-1))
    ...
    def get_transaction_stats(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        """
        if df.height == 0:
            return {"total_transactions": 0, "total_frames": 0, "avg_frames_per_transaction": 0}
        
        transaction_stats = df.group_by(self.config.transaction_id_col).len().sort(self.config.transaction_id_col)
...
def split_usb_transactions(df: pl.DataFrame, 
                          config: Optional[TransactionSplitterConfig] = None) -> pl.DataFrame:
    """
